...
OUTPUT_DIR = Path("experiments/checkpoints/w1_unfiltered")

EPOCHS = 2
LEARNING_RATE = 5e-5
SEED = 42

# Process one utterance at a time.
# This is deliberately conservative for CPU training.
...
def prepare_sample(processor, row):
    ...
    # W1 target = Parakeet's UNFILTERED transcription.
...
def main():
    print("=" * 70)
    print("W1 — WHISPER LoRA / UNFILTERED PARAKEET")
    print("=" * 70)
    ...
    val_df = dev_df[
        dev_df["id"].astype(str).isin(val_ids)
    ].copy()
